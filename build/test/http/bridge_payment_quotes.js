...
describe("bitpay inbound quotes http/json api", function () {
  ...
  it("the fee currency should be the same as the external destination and ripple source currencies", function () {
    assert(quote.fee.currency === quote.external.destination.currency && quote.fee.currency === quote.ripple.source.currency);
  });

  before(function (done) {
    httpClient(app).post("/bitpay/bridge_quotes").send({
      destination: {
        amount: 100,
        currency: "XRP",
